...
void GoSUMO::act(uint8_t motion, uint16_t speed){
  if(motion == FORWARD){
    ledcWrite(_channel_1a, 0); ledcWrite(_channel_1b, speed); //Moto-1,CCW
    ledcWrite(_channel_3a, 0); ledcWrite(_channel_3b, speed); //Moto-3,CCW
    ledcWrite(_channel_2a, speed); ledcWrite(_channel_2b, 0); //Moto-2,CW
    ledcWrite(_channel_4a, speed); ledcWrite(_channel_4b, 0); //Moto-4,CW 
  }
...
void GoSUMO::linetracking(uint8_t motion, uint16_t speed, uint16_t wheel_difference){
  if(motion == FORWARD){
    ledcWrite(_channel_1a, 0); ledcWrite(_channel_1b, speed); //Moto-1, Left side wheel
    ledcWrite(_channel_3a, 0); ledcWrite(_channel_3b, speed); //Moto-3, Left side wheel
    ledcWrite(_channel_2a, speed); ledcWrite(_channel_2b, 0); //Moto-2, Righ side wheel
    ledcWrite(_channel_4a, speed); ledcWrite(_channel_4b, 0); //Moto-4, Righ side wheel
  }
  else if(motion == BACKWARD){
    ledcWrite(_channel_1a, speed); ledcWrite(_channel_1b, 0); //Moto-1
    ledcWrite(_channel_3a, speed); ledcWrite(_channel_3b, 0); //Moto-3
    ...
    ledcWrite(_channel_1a, 0); ledcWrite(_channel_1b, (speed - wheel_difference)); //Moto-1, Left side wheel
    ledcWrite(_channel_3a, 0); ledcWrite(_channel_3b, (speed - wheel_difference)); //Moto-3, Left side wheel
    ledcWrite(_channel_2a, speed); ledcWrite(_channel_2b, 0); //Moto-2, Righ side wheel
    ledcWrite(_channel_4a, speed); ledcWrite(_channel_4b, 0); //Moto-4, Righ side wheel
  }
  else if(motion == GO_RIGHT){
    ledcWrite(_channel_1a, 0); ledcWrite(_channel_1b, speed); //Moto-1, Left side wheel
    ledcWrite(_channel_3a, 0); ledcWrite(_channel_3b, speed); //Moto-3, Left side wheel
    ledcWrite(_channel_2a, (speed - wheel_difference)); ledcWrite(_channel_2b, 0); //Moto-2, Righ side wheel
    ledcWrite(_channel_4a, (speed - wheel_difference)); ledcWrite(_channel_4b, 0); //Moto-4, Righ side wheel
  }
}
...
byte IR3CH::ReadTCRT(uint16_t TH, bool DEBUG){
  byte _bTrace = 0;
...
/*** UAV ***/
UAV::UAV(uint8_t pin, uint8_t channel){
  _pin = pin; 
  _channel = channel;

  pinMode(_pin, OUTPUT);

  ledcSetup(_channel, _m_frequency, _m_resolution);
  ledcAttachPin(_pin, _channel);    
}

void UAV::act(uint16_t speed){
  ledcWrite(_channel, speed);
}

void UAV::stop(){  ledcWrite(_channel, 0); }